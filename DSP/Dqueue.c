//Program - 4 Dqueue
#include<stdio.h>
#define max 10
int DQ[max];
void main()
{
	int choice,ele,Remove;
	int *front;
	int *rear;
	
	void DQ_Insert(int *,int *,int ele);
	int DQ_Delete(int *,int *);
	void DQ_Display(int *,int *);

	front=(int *)malloc(sizeof(int));
	rear=(int *)malloc(sizeof(int));

	*front=-1;
	*rear=-1;

	while(choice!=4)
	{
		printf("1 . Insert \n");
		printf("2 . Delete \n");
		printf("3 . Display \n");
		printf("Enter Your Choice: ");
		scanf("%d",&choice);

		switch(choice)
		{
		case 1:{
				printf("Enter Your Element: ");
				scanf("%d",&ele);
				DQ_Insert(front,rear,ele);
				break;
			   }
		case 2:{
				Remove=DQ_Delete(front,rear);
				printf("Removed Value is %d \n",Remove);
				break;
			   }
		case 3:{
				DQ_Display(front,rear);
				break;
			   }
		}	
	}
}
void DQ_Insert(int *front,int *rear,int ele)
{
	if((*front)==-1 && (*rear)==-1)
	{
		(*front)=(*rear)=0;
		DQ[(*rear)]=ele;
	}
	else
	{
			if((*rear)==max-1)
			{
				printf("Queue is full");
			}
			else
			{
				(*rear)=(*rear)+1;
				DQ[(*rear)]=ele;
			}
	}
}
int DQ_Delete(int *front,int *rear)
{	int Remove,choice;
     printf("Enter 1 remove Front to end \n");
	 printf("Enter 2 remove End to front \n");
     printf("enter your choice : ");
     scanf("%d",&choice);
	
		if(choice==1)
		{	
			if((*front)==-1 && (*rear)==-1)
			{
				printf("Queue id Empty");
			}
			else{
			Remove=DQ[(*front)];
			(*front)=(*front)+1;}
		}
		else
		{	if((*front)==-1 && (*rear)==-1)
			{
				printf("Queue id Empty");
			}
			else{
				Remove=DQ[(*rear)];
				(*rear)=(*rear)-1;
			}	
		}
	return Remove;
}
void DQ_Display(int *front,int *rear)
{	
	int i,choice;
    printf("Enter 1 disply Front to end \n");
	printf("Enter 2 disply End to Front \n");
	printf("enter your choice : ");
    scanf("%d",&choice);
	if(choice==1)
    {	if((*front)==-1 && (*rear)==-1)
			{
				printf("Queue id Empty");
			}
		else{
			for(i=(*front);i<=(*rear);i++)
			{
				printf("%d \n",DQ[i]);
			}
		}
    }
    else
	{	if((*front)==-1 && (*rear)==-1)
			{
				printf("Queue id Empty");
			}
		else{
			for(i=(*rear);i>=(*front);i--)
			{
				printf("%d \n",DQ[i]);
			}
		}
	}
}
